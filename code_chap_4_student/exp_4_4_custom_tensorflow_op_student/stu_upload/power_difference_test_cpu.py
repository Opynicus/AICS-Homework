...
def power_difference_op(input_x,input_y,input_pow):
    with tf.Session() as sess:
        ...
        phx = tf.placeholder(tf.float32, name="x")
        phy = tf.placeholder(tf.float32, name="y")
        phpow = tf.placeholder(tf.float32, name="pow")
        out = tf.power_difference(phx, phy, phpow)
        return sess.run(out, feed_dict = {phx:input_x, phy:input_y, phpow:input_pow})
