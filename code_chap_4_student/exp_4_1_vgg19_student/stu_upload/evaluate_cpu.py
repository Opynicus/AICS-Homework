...
def net(data_path, input_image):
    ...
    for i, name in enumerate(layers):
        if name[:4] == 'conv':
            ...
            bias = bias.flatten()
            #print(current.shape[0], current.shape[1], current.shape[2], current.shape[3])
            ...
            bias = bias.flatten()
            #print(current.shape[0], current.shape[1], current.shape[2], current.shape[3])
            #print(kernels.shape[0], kernels.shape[1], kernels.shape[2], kernels.shape[3])
            current = tf.reshape(current, (current.shape[0], -1))
            kernels = tf.reshape(kernels, (-1, kernels.shape[3]))
...
def _conv_layer(input, weights, bias):
    ...
    return tf.nn.bias_add(tf.nn.conv2d(input, weights, 1, "SAME"), bias)
