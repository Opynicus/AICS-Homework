...
def loss_function(net, content_features, style_features, content_weight, style_weight, tv_weight, preds, batch_size):
    ...
    # 计算内容损失
    # content_loss
    content_size = _tensor_size(content_features[CONTENT_LAYER])*batch_size   #H*W*C*N
    assert _tensor_size(content_features[CONTENT_LAYER]) == _tensor_size(net[CONTENT_LAYER])
    content_loss = content_weight * tf.pow(tf.norm(net[CONTENT_LAYER] - content_features[CONTENT_LAYER], ord=2), 2) / content_size
    ...
    for style_layer in STYLE_LAYERS:
        ...
        style_losses.append(tf.pow(tf.norm(grams - style_gram, ord=2), 2) / size)
