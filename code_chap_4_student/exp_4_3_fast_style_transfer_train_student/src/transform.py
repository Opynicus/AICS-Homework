...
def net(image, type=0):
    ...
    r1 = _residual_block(conv3, 3 , 1)
    r2 = _residual_block(r1, 3)
    r3 = _residual_block(r2, 3)
    r4 = _residual_block(r3, 3)
    r5 = _residual_block(r4, 3)
    convTrans1 = _conv_tranpose_layer(r5, 64, 3, 2)
    convTrans2 = _conv_tranpose_layer(convTrans1, 32, 3, 2)
    conv4 = _conv_layer(convTrans2, 3, 9, 1)
...
def _conv_layer(net, num_filters, filter_size, strides, relu=True, type=0):
    ...
    # TODO：准备好权重的初值
    weights_init = _conv_init_vars(net, num_filters, filter_size) # [k,k,cin,cout]
...
def _conv_tranpose_layer(net, num_filters, filter_size, strides, type=0):
    ...
    batch, h, w, channels = [i.value for i in net.get_shape()]
    strides_shape = [1, strides, strides ,1]
    h = h * strides
    w = w * strides
    output_shape = [batch, h, w, num_filters]
...
def _residual_block(net, filter_size=3, type=0):
    ...
    tmp = _conv_layer(net, 128, filter_size, 1, True)
    temp = _conv_layer(tmp, 128, filter_size, 1, False)
    net = tf.nn.relu(net+temp)
    return net
