...
class ContentLossLayer(object):
    ...
    def forward(self, input_layer, content_layer):
        ...
        loss = np.sum(np.square(input_layer - content_layer)) / (input_layer.shape[0]*input_layer.shape[1]*input_layer.shape[2]*input_layer.shape[3]*2)
        return loss
...
class StyleLossLayer(object):
    ...
    def backward(self, input_layer, style_layer):
        ...
        for idxn in range(input_layer.shape[0]):
            # TODO： 计算风格损失的反向传播
            bottom_diff[idxn, :, :] = np.dot(self.gram_input[idxn,:,:]-self.gram_style, self.input_layer_reshape[idxn,:,:]) / (input_layer.shape[0]*self.div)
