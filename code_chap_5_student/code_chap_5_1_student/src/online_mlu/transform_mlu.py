...
def run_ori_power_diff_pb():
    ...
    with g.as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            start_time = time.time()
            ret =sess.run(output_tensor,feed_dict={input_tensor_pow:input_pow,input_tensor:[X]})
            end_time = time.time()
...
def run_numpy_pb():
    ...
    with g.as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            output_tensor = sess.graph.get_tensor_by_name('add_37:0')
            ret = sess.run(output_tensor, feed_dict={input_tensor_new:output,input_tensor:[X]})
            end_time = time.time()
